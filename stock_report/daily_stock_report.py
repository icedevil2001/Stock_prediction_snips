from util import send_email, stock_reporter
import datetime
...
with PdfPages('stock_daily_report.pdf') as pdf:
    for tinker in set(config['stock_list']) :
        stock = (stock_reporter.convert_tinker(tinker))
        print(stock)
        try:
            df = stock_reporter.process_df(stock, STARTDATE, stock_reporter.config.MACD_long() )
            # print(df)
